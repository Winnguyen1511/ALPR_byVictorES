...
def autoGetSizeImg(xmlDir, cropDir):
    global testData
    if xmlDir == '.':
        xmlDirSuffix = ""
    else:
    ...
    if cropDir == '.':
        ...
        if not cropDir.endswith('/'):
            ...
            cropDirSuffix = cropDir
    lstXml = [xml for xml in os.listdir(xmlDir) if xml.endswith('.xml')]
    lstXml = sorted(lstXml, key=getXmlNum)
    ...
    for annotation in lstXml:
        root = ET.parse(annotation).getroot()
        lstRoot.append(root)
    lstQuery = list(zip(lstXml, lstRoot))
    bar = IncrementalBar('Getting Size', max=len(lstQuery))
    for i in range(0, len(lstQuery)):
        # print("Cropping objects from ", lstQuery[i][IMAGE])
        res = getSizeImg(lstQuery[i])
        if not res:
            print("Error: getsize error!")
            return False
        bar.next()
    bar.finish()

    savefile = open(cropDirSuffix+"data", 'w')
    for key in testData.keys():
        savefile.write(key+" "+testData[key]+"\n")
    savefile.close()

    print("> Auto getsize %d images completed!"%(i+1))
    return True
...
def main():
    ...
    res = autoGetSizeImg(XMLAnnotations, Cropped)
